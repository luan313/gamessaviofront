...
export function ReviewCard({
  ...
  return (
    <Card className="group relative overflow-hidden border-border bg-card mb-4">
      {/* IMAGEM DE FUNDO */}
      {gameImage && (
        <>
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 space-y-2">
            <div className="flex justify-between items-start">
              <div>
                <Link href="#" className="font-bold text-lg text-foreground hover:text-blue-400 transition-colors">
                  {userName}
                </Link>
                <div className="flex items-center gap-3 mt-1">
                  <div className="flex items-center gap-1 bg-secondary/50 px-2 py-0.5 rounded border border-border">
              ...
              {isOwnReview && (
                <Button variant="outline" size="sm" className="bg-transparent border-border text-foreground hover:bg-secondary">Editar</Button>
              )}
            </div>

            <div className="relative pl-3 border-l-2 border-border">
              <Quote className="absolute -left-1 -top-2 h-6 w-6 text-muted-foreground/20 -z-10 transform -scale-x-100" />
              <p className="text-muted-foreground italic font-medium">"{comment}"</p>
            </div>
          </div>
        </div>
      </CardContent>
